...
\emph{AST}, no de movimiento. En un \emph{behaviour step} miden distancia horizontal
(al \emph{player} o al \emph{spawn anchor}) y, según el resultado, deciden qué
...
\begin{center}
  ...
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{4cm} >{\raggedright\arraybackslash}X@{}}
    ...
    Mide distancia horizontal al \emph{player} y continúa por \texttt{thenBranch} o
    \texttt{elseBranch}. \\
    \texttt{IfNearSpawn} &
    Mide proximidad horizontal al \emph{spawn anchor} y continúa por
...
\textbf{Catálogo del \emph{run}.} Ver tabla~\ref{tab:catalogo-modos} y
\texttt{BootstrapRun} (sección~\ref{subsec:solucion-catalogo}). Ante fallo de generación o
de resolución de hint, el motor conserva el \emph{fallback} del \emph{slot} o el
\emph{default} del \texttt{kind} y la partida no se aborta.

\textbf{Niveles generados por LLM.} Con \envvar{WONDERBOY_GENERATE_LEVELS} activo, el
adaptador consulta la API del LLM (Anthropic) una vez por perfil (intro, desafío, jefe) y devuelve
...
\textbf{\emph{Hints} en JSON fijos.} En los niveles escritos a mano, el autor describe el
comportamiento de un \emph{enemy} con \texttt{behaviour\-Hint}. Al arrancar,
\texttt{BehaviourResolver\-Port} traduce ese texto a \emph{archetype} y
...
Complementariamente, durante pruebas manuales en el juego se puede alternar con
\texttt{F1} la superposición de \emph{hitboxes} de todas las \emph{entities} y
de las plataformas, para verificar en pantalla la alineación entre sprites y
cajas de colisión.
