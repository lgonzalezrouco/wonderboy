...
--   1. **Costo cero en runtime**: un `newtype` es idéntico a su tipo interno
--      en memoria; GHC lo elimina durante la compilación.
--   2. **Seguridad de tipos**: aunque `Position` y `Velocity` contienen ambas un
...
-- `Show`    → permite imprimir posiciones en GHCi o en tests: `show (Position (1,2)) = "Position (1.0,2.0)"`.
