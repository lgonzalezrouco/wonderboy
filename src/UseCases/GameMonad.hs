...
newtype GameM a = GameM
  ...
  deriving newtype
    ...
    -- ^ Habilita `get :: GameM GameState` (leer estado), `put :: GameState -> GameM ()` (reemplazar)
    --   y `modify :: (GameState -> GameState) -> GameM ()` (actualizar).
    , MonadError GameError
    -- ^ Habilita `throwError :: GameError -> GameM a` (abortar con error) y
    --   `catchError :: GameM a -> (GameError -> GameM a) -> GameM a` (recuperarse).
    )
